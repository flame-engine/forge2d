...
final class PrismaticJoint extends Joint {
  ...
  @internal
  const PrismaticJoint.internal(super.world, super.index1, super.wg)
    : super.internal();

  /// Whether the spring is enabled.
  bool get springEnabled => rawBox2D.prismaticJointIsSpringEnabled(index1, wg);

  set springEnabled(bool value) =>
      rawBox2D.prismaticJointEnableSpring(index1, wg, enabled: value);

  /// The spring stiffness in hertz.
  double get springHertz => rawBox2D.prismaticJointGetSpringHertz(index1, wg);

  set springHertz(double value) =>
      rawBox2D.prismaticJointSetSpringHertz(index1, wg, value);

  /// The spring damping ratio.
  double get springDampingRatio =>
      rawBox2D.prismaticJointGetSpringDampingRatio(index1, wg);

  set springDampingRatio(double value) =>
      rawBox2D.prismaticJointSetSpringDampingRatio(index1, wg, value);

  /// The target translation for the spring, in meters.
  double get targetTranslation =>
      rawBox2D.prismaticJointGetTargetTranslation(index1, wg);

  set targetTranslation(double value) =>
      rawBox2D.prismaticJointSetTargetTranslation(index1, wg, value);

  /// Whether the translation limit is enabled.
  bool get limitEnabled => rawBox2D.prismaticJointIsLimitEnabled(index1, wg);

  set limitEnabled(bool value) =>
      rawBox2D.prismaticJointEnableLimit(index1, wg, enabled: value);

  /// The lower translation limit, in meters.
  double get lowerLimit => rawBox2D.prismaticJointGetLowerLimit(index1, wg);

  /// The upper translation limit, in meters.
  double get upperLimit => rawBox2D.prismaticJointGetUpperLimit(index1, wg);

  /// Sets the translation limits, in meters.
  void setLimits({required double lower, required double upper}) =>
      rawBox2D.prismaticJointSetLimits(index1, wg, lower, upper);

  /// Whether the motor is enabled.
  bool get motorEnabled => rawBox2D.prismaticJointIsMotorEnabled(index1, wg);

  set motorEnabled(bool value) =>
      rawBox2D.prismaticJointEnableMotor(index1, wg, enabled: value);

  /// The motor speed, in meters per second.
  double get motorSpeed => rawBox2D.prismaticJointGetMotorSpeed(index1, wg);

  set motorSpeed(double value) =>
      rawBox2D.prismaticJointSetMotorSpeed(index1, wg, value);

  /// The maximum motor force, in newtons.
  double get maxMotorForce =>
      rawBox2D.prismaticJointGetMaxMotorForce(index1, wg);

  set maxMotorForce(double value) =>
      rawBox2D.prismaticJointSetMaxMotorForce(index1, wg, value);

  /// The current motor force, in newtons.
  double get motorForce => rawBox2D.prismaticJointGetMotorForce(index1, wg);

  /// The current translation along the joint axis, in meters.
  double get translation => rawBox2D.prismaticJointGetTranslation(index1, wg);

  /// The current speed along the joint axis, in meters per second.
  double get speed => rawBox2D.prismaticJointGetSpeed(index1, wg);
}
